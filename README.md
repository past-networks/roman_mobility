...
_de Soto, P. et al. (2025) A High-Resolution Dataset of Roads of the Roman Empire: Itiner-e static version 2024. Zenodo. https://doi.org/10.5281/zenodo.17122148`_

Related publication: _de Soto, P., Pažout, A., Brughmans, T. et al. Itiner-e: A high-resolution dataset of roads of the Roman Empire. Sci Data 12, 1731 (2025). https://doi.org/10.1038/s41597-025-06140-z_
...
Related publication: _Filet, C., Laroche, C., Coto-Sarmiento, M., & Bongers, T. (2025). As the water flows: A method for assessing river navigability in the past. Journal of Archaeological Science, 182, 106315. https://doi.org/10.1016/j.jas.2025.106315_

### Sea lanes

The data is on https://itiner-e.org/ with type = sea lane.

Related publication: _Gal, D., Saaroni, H., & Cvikel, D. (2023). Mappings of Potential Sailing Mobility in the Mediterranean During Antiquity. Journal of Archaeological Method and Theory, 30(2), 397–448. https://doi.org/10.1007/s10816-022-09567-5_


### Latin inscriptions

_Sobotkova, A., Heřmánková, P., & Kaše, V. (2025). origo (v1.0) [Data set]. Zenodo. https://doi.org/10.5281/zenodo.14604222_

Related publication: _Sobotkova, A., Heřmánková, P., & Kaše, V. Soldier Mobility in the Roman Empire from Birth-to-Death Locations in Latin Epigraphs. TBA._
...
_Hanson, J. W. (2016). Cities Database (OXREP databases). Version 1.0. Accessed (date): <http://oxrep.classics.ox.ac.uk/databases/cities/>. DOI: <https://doi.org/10.5287/bodleian:eqapevAn8>_

Related publication: _Hanson J. W., An urban geography of the Roman world, 100 BC to AD 300. Oxford: Archaeopress; 2016. http://oxrep.classics.ox.ac.uk/oxrep/docs/Hanson2016/Hanson2016_Cities_OxREP.csv_

3) **Population estimates**

_Hanson J. W, Ortman S. G., A systematic method for estimating the populations of Greek and Roman settlements. J Roman Archaeol. 2017;30: 301–324._
...
2. **`mobilitydata_analysis.Rmd`**: Full mobility analysis pipeline for the `origo` dataset [responsible: Wójtowicz]

   Covers the following steps in order:
   - **Data loading & examination** — loads the `origo_geo.parquet` dataset and Roman province boundaries, converts WKT geometry columns to `sf` objects, and inspects distributions of dating, gender, status, occupation, and inscription type
   - **Top 10 route network** — filters valid origin–destination pairs, excludes self-loops (same origin and findspot), counts people per route, and produces three network visualisations: force-directed (Fruchterman-Reingold), hierarchical (sources left → destinations right), and a grid of individual route cards. Also checks for bidirectional routes.
   - **Hexagon grids** — builds discrete global hexagon grids (dggridR) at resolution 12 (~96 km²) and 13 (~31 km²) for both origin and findspot locations; colour encodes number of people per cell
   - **Network on map (styled)** — aggregates top 30 routes as straight lines onto the province map; blue dots = origins, red dots = destinations
   - **Network on map (simple)** — uniform red lines for the top 30 routes; includes raw geometry inspection and exports `top_30_mobility_routes.csv`
